import { compactPersistedHistory } from './profile-storage.js';
...
export function createDataController({
  ...
  async function reportChartDiscoveryCandidates(rows, historyId = '') {
    if (!isAuthorized() || typeof collectChartDiscoveryCandidates !== 'function') return;
    const candidates = collectChartDiscoveryCandidates(state.rankTables, rows);
    if (!candidates.length) return;
    try {
      await rpc('report_chart_discovery_candidates', {
        p_candidates: candidates,
        p_history_id: String(historyId || '').trim()
      });
    } catch (error) {
      console.warn('Chart discovery candidate report failed', error);
    }
  }
  ...
  async function applyTrackerContent(content, trackerName, toastMessage = '데이터 업로드 완료') {
    ...
    const runApply = async () => {
      if (isAuthorized()) {
        const prevProfile = cloneJson(state.profile);
        const prevSelectedHistoryId = state.selectedHistoryId;
        const prevHistorySectionOpen = cloneJson(state.historySectionOpen) || createHistorySectionState();
        const prevHistoryAnimateDetail = !!state.historyAnimateDetail;
        ...
        if (totalChanges) {
          ...
          state.profile.history = compactPersistedHistory(state.profile.history);
        ...
        const discoveryHistoryId = latestHistoryId(state.profile.history);
        try {
          ...
          state.selectedHistoryId = prevSelectedHistoryId;
          state.historySectionOpen = prevHistorySectionOpen;
          state.historyAnimateDetail = prevHistoryAnimateDetail;
          rebuildViews();
          render();
        ...
        try {
          if (typeof refreshProfile === 'function') {
            await refreshProfile({ showBusy: false, timeoutMs: 10000 });
          }
        } catch (error) {
          console.error('TSV cloud refresh failed', error);
          render();
          showToast(`DB 저장은 완료됐지만 재조회에 실패했습니다: ${error.message || error}`);
          return;
        }
        await reportChartDiscoveryCandidates(rows, discoveryHistoryId);
