  ...
  body.innerHTML = notices.length
    ? notices.map((notice) => {
      ...
      return `<article class="notice-history-item">
        ...
        <div class="notice-history-item-content">
...
export function renderSettings(ctx) {
  ...
  const discoveryCard = $('settingsAdminDiscoveryCard');
  const discoveryList = $('settingsAdminDiscoveryList');
  ...
  if (discoveryCard) discoveryCard.classList.toggle('hidden', !isAdmin);
  if (publishBtn) {
  ...
  if (discoveryList) {
    const candidates = Array.isArray(ctx.state.chartDiscoveryCandidates) ? ctx.state.chartDiscoveryCandidates : [];
    discoveryList.innerHTML = candidates.length
      ? candidates.map((row) => {
        const reporter = [row.reporterDjName, row.reporterInfinitasId].filter(Boolean).join(' / ');
        const meta = [
          row.tableKey,
          `[${row.chartType}]`,
          row.noteCount ? `${row.noteCount} notes` : '',
          row.seenCount ? `${row.seenCount} reports` : ''
        ].filter(Boolean).join(' · ');
        return `<article class="settings-discovery-item">
          <div class="settings-discovery-main">
            <strong>${esc(row.songTitle || '-')}</strong>
            <span>${esc(meta)}</span>
            ${reporter ? `<span>최초 제보: ${esc(reporter)}</span>` : ''}
          </div>
          <button type="button" class="small-btn danger" data-discovery-promote="${esc(row.id)}">pending 등록</button>
        </article>`;
      }).join('')
      : '<div class="settings-admin-status">대기 중인 신규 곡 후보가 없습니다.</div>';
  }
}
