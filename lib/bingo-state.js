...
export function selectBingoBoard(previousState, boardId) {
  const previous = normalizeBingoStateShape(previousState);
  ...
  const activeBoard = previous.savedBoards.find((board) => String(board?.id || '') === targetId);
  if (!activeBoard) return previous;
  return {
    ...previous,
    activeBoardId: targetId,
    published: cloneJson(activeBoard),
    selectedGoalId: '',
    selectedCellIndex: -1
  };
}

export function removeSavedBoardFromState(previousState, boardId, options = {}) {
  const previous = normalizeBingoStateShape(previousState);
  const targetId = String(boardId || '').trim();
  if (!targetId) return previous;
  const savedBoards = previous.savedBoards.filter((board) => String(board?.id || '') !== targetId);
  const preferDraft = options.preferDraft === true;
  const activeBoardId = preferDraft ? '' : String(savedBoards[0]?.id || '').trim();
  const published = activeBoardId
    ? cloneJson(savedBoards.find((board) => String(board?.id || '') === activeBoardId) || null)
    : null;
  return {
    ...previous,
    savedBoards,
