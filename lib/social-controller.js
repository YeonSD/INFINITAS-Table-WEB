export function createSocialController(env) {
  ...
  function followingPeerIds() {
    return new Set(
      (state.social.overviewRows || [])
        .filter((row) => row.relation_type === 'follow' && String(row.direction || '') === 'following')
        .map((row) => String(row.peer_user_id || ''))
    );
  }

  function isMutualFollow(peerUserId) {
    if (!peerUserId) return false;
    const rows = (state.social.overviewRows || []).filter((row) => row.relation_type === 'follow' && String(row.peer_user_id || '') === String(peerUserId));
    const following = rows.some((row) => String(row.direction || '') === 'following');
    const follower = rows.some((row) => String(row.direction || '') === 'follower');
    return following && follower;
  }

  function followersPopupPosition(anchorRect) {
    const popupWidth = 340;
    const popupHeight = 420;
    const viewportWidth = Math.max(window.innerWidth || 0, popupWidth + 32);
    const viewportHeight = Math.max(window.innerHeight || 0, popupHeight + 32);
    const baseLeft = anchorRect ? Number(anchorRect.left || 0) : (viewportWidth - popupWidth) / 2;
    const baseTop = anchorRect ? Number(anchorRect.bottom || 0) + 8 : Math.max(24, (viewportHeight - popupHeight) / 3);
    return {
      left: Math.min(Math.max(16, baseLeft), Math.max(16, viewportWidth - popupWidth - 16)),
      top: Math.min(Math.max(16, baseTop), Math.max(16, viewportHeight - popupHeight - 16))
    };
  }
  ...
  async function respondFollow(requestId, accept) {
    try {
      const requestRow = (state.social.overviewRows || []).find((row) => String(row.request_id || '') === String(requestId || ''));
      const relatedFeedEventIds = (state.social.feedItems || [])
        .filter((item) => String(item?.event_type || '') === 'follow_request_received' && String(item?.payload?.request_id || '') === String(requestId || ''))
        .map((item) => String(item.id || ''))
        .filter(Boolean);
      const peerUserId = String(requestRow?.peer_user_id || '');
      const peerLabel = String(requestRow?.dj_name || '상대');
      await rpc('respond_follow_request', { p_request_id: requestId, p_accept: accept });
      if (relatedFeedEventIds.length) {
        await Promise.all(relatedFeedEventIds.map((eventId) => rpc('dismiss_feed_event', { p_event_id: eventId }).catch(() => null)));
      }
      await syncSocial();
      if (!accept) {
      ...
      if (peerUserId && isMutualFollow(peerUserId)) {
        showToast('팔로우를 수락하였습니다! 이제 서로 맞팔입니다.');
        return;
      }
      if (!peerUserId) {
        showToast('팔로우를 수락하였습니다.');
        return;
      }
      const confirmed = window.confirm(`팔로우를 수락하였습니다! ${peerLabel}님에게도 팔로우 요청을 보내시겠습니까?`);
      if (!confirmed) {
        showToast('팔로우를 수락하였습니다.');
        return;
      }
      await sendFollow(peerUserId, {
        peerLabel,
        closeSearchDialog: false,
        toastMode: 'reciprocal'
      });
    } catch (error) {
  ...
  async function respondBingo(transferId, accept) {
    ...
    try {
      ...
      if (accept && currentSavedBoards().length > 5) {
        ...
        syncPublishedFromSavedBoards();
        await saveBingoStateToCloud(state.auth.user, state.profile, 'bingo-transfer-accepted');
      }
  ...
  async function sendFollow(targetUserId, options = {}) {
    try {
      const status = String(await rpc('send_follow_request', { p_target_user_id: targetUserId }) || '').trim();
      if (options.closeSearchDialog !== false) $('socialFollowAddDialog')?.close('done');
      await syncSocial();
      if (options.showToast !== false) {
        if (options.toastMode === 'reciprocal') {
          if (status === 'auto_accepted' || status === 'already_following' || followingPeerIds().has(String(targetUserId || ''))) {
            showToast('팔로우를 수락하였습니다! 이제 서로 맞팔입니다.');
          } else {
            showToast('팔로우를 수락하였습니다! 상대에게도 팔로우 요청을 보냈습니다.');
          }
        } else if (status === 'auto_accepted') {
          showToast('팔로우가 바로 수락되었습니다.');
        } else if (status === 'already_following') {
          showToast('이미 팔로우 중입니다.');
        } else {
          showToast('팔로우 요청을 보냈습니다.');
        }
      }
      return status;
    } catch (error) {
  ...
  function openFollowersPopup(anchorRect = null) {
    state.socialFollowersPopup = {
      ...state.socialFollowersPopup,
      ...followersPopupPosition(anchorRect),
      open: true
    };
    render();
  }
  ...
  function closeFollowersPopup() {
    if (!state.socialFollowersPopup?.open) return;
    state.socialFollowersPopup = {
      ...state.socialFollowersPopup,
      open: false
    };
    ...
    state.profile.socialSettings = normalizeSocialSettings({
      ...
      shareDataScope: $('settingShareAllData')?.checked
        ...
        : $('settingShareNone')?.checked
          ? ['none']
          : ['graphs'],
      shareRankTableWithMutuals: $('settingShareRankTableWithMutuals')?.checked !== false,
      goalTransferPolicy: 'disabled',
      rivalPolicy: 'followers',
      bannerDataUrl: currentSocialSettings().bannerDataUrl || ''
    });
    await saveSocialSettingsToCloud(state.auth.user, state.profile, 'settings-save');
  ...
  async function saveSocialBannerEditor() {
    ...
    try {
      ...
      await saveSocialSettingsToCloud(state.auth.user, state.profile, 'social-banner');
      render();
