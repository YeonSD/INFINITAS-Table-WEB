...
import { buildAccountStatePatchPayload, buildFullAccountStatePayload, buildUserProfilePayload } from './profile-storage.js';
...
export async function saveProfileToCloud(user, profile, reason = 'manual') {
  ...
  await saveFullAccountStateToCloud(user, profile, reason);
}
...
export async function saveFullAccountStateToCloud(user, profile, reason = 'manual') {
  const client = getSupabaseClient();
  if (!client || !user || !profile) return;
  const statePayload = buildFullAccountStatePayload(user, profile, reason, nowIso());
