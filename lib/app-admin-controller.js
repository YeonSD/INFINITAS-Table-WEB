...
export function createAdminController({
  ...
  function normalizeDiscoveryCandidate(row) {
    ...
    const type = String(row.chart_type || '').trim().toUpperCase();
    const tableKey = String(row.table_key || '').trim();
    if (!id || !title || !type || !tableKey) return null;
    return {
      id,
      chartKey: String(row.chart_key || '').trim(),
      tableKey,
      ...
      firstSeenAt: String(row.first_seen_at || row.created_at || '').trim(),
      lastSeenAt: String(row.last_seen_at || row.updated_at || '').trim(),
      reporterDjName: String(row.reporter_dj_name || '').trim(),
      reporterInfinitasId: String(row.reporter_infinitas_id || '').trim()
    };
  }

  async function refreshChartDiscoveryCandidates(options = {}) {
    if (!state.auth.isAdmin) {
      state.chartDiscoveryCandidates = [];
      return;
    }
    const renderAfter = options.renderAfter !== false;
    const silent = options.silent === true;
    try {
      const rows = await rpc('get_chart_discovery_candidates', { p_limit: 80 });
      state.chartDiscoveryCandidates = (Array.isArray(rows) ? rows : [])
        .map(normalizeDiscoveryCandidate)
        .filter(Boolean);
    } catch (error) {
      if (!silent) showToast(`신규 곡 후보 불러오기 실패: ${describeRemoteError(error)}`);
    }
    if (renderAfter) render();
  }
  ...
  async function promoteDiscoveryCandidate(candidateId = '') {
    if (!state.auth.isAdmin || !state.auth.user) return showToast('관리자 계정만 신규 곡 후보를 등록할 수 있습니다.');
    const candidate = (state.chartDiscoveryCandidates || []).find((row) => String(row.id || '') === String(candidateId || ''));
    if (!candidate) return;
    await withBusyOverlay(
      '신규 곡 후보 등록 중...',
      '후보 곡을 미출시 pending 상태로 chart_metadata에 등록하고 있습니다.',
      async () => {
        const row = await rpc('admin_promote_chart_discovery_candidate', {
          p_candidate_id: candidate.id,
          p_category: '미정',
          p_source_sort_index: 0
        });
        applyChartMetadataRowToLocalState(Array.isArray(row) ? row[0] : row);
        await refreshChartDiscoveryCandidates({ renderAfter: false, silent: true });
        markSnapshotPublishNeeded('신규 곡 후보가 pending 상태로 등록되었습니다. 변경사항 배포를 누르면 관리자 스냅샷에 반영됩니다.');
        render();
        showToast('신규 곡 후보를 pending 상태로 등록했습니다.');
      }
    );
  }
