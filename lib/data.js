...
import { createEmptyBingoState, normalizeBingoStateShape } from './bingo-state.js';
...
function normalizeGoalSnapshot(rawGoal) {
  ...
  if (!table || !title || !['CLEAR', 'SCORE', 'RANK'].includes(kind)) return null;
  const snapshot = {
...
export function normalizeBingoState(rawState) {
  return normalizeBingoStateShape(rawState, {
    normalizeCell: normalizeGoalSnapshot
  });
}
