...
const SUPPRESSED_TRACKER_ONLY_CHARTS = [
  ['SP12H', 'REVOLVER', 'L'],
  ['SP12H', 'GUILTY', 'L'],
  ['SP12H', 'MAD ATTACK', 'L'],
  ['SP12H', 'Override', 'L'],
  ['SP12H', 'お米の美味しい炊き方、そしてお米を食べることによるその効果。', 'L']
];

const SUPPRESSED_TRACKER_ONLY_CHART_KEYS = new Set(
  SUPPRESSED_TRACKER_ONLY_CHARTS.map(([tableKey, title, type]) => `${tableKey}|${titleKey(title)}|${type}`)
);

function isSuppressedTrackerOnlyChart(tableKey, title, type) {
  return SUPPRESSED_TRACKER_ONLY_CHART_KEYS.has(`${String(tableKey || '').trim()}|${titleKey(title)}|${String(type || '').trim().toUpperCase()}`);
}
...
export function buildViews(rankTables, songRadarCatalog, rows, options = {}) {
  ...
  Object.entries(rankTables || {}).forEach(([tableName, tableData]) => {
    ...
    (tableData.categories || []).forEach((category) => {
      (category.items || []).forEach((item) => {
        ...
        const releaseStatus = String(item?.data?.releaseStatus || 'live').trim().toLowerCase() || 'live';
        if (!title || !/^[HAL]$/.test(type)) return;
        if (releaseStatus === 'pending_release') {
          pendingKeys.add(`${titleKey(title)}|${type}`);
        }
      });
    });
    ...
    (tableData.categories || []).forEach((category) => {
      ...
      (category.items || []).forEach((item) => {
        ...
        const type = String(item?.data?.type || 'A').trim().toUpperCase();
        const releaseStatus = String(item?.data?.releaseStatus || 'live').trim().toLowerCase() || 'live';
        const isAdminPending = isAdmin && releaseStatus === 'pending_release';
        if (!title || !/^[HAL]$/.test(type)) return;
        if (releaseStatus === 'pending_release' && !isAdmin) return;
        const row = findRowByTitle(rowIndex, title);
        if (hasTracker && !row && !isAdminPending) return;
        if (hasTracker && row && !isAdminPending) {
        ...
        const chart = {
          ...
          type,
          releaseStatus,
          isPendingRelease: releaseStatus === 'pending_release',
          ...stats,
    ...
    if (hasTracker && expectedLevel > 0) {
      ...
      (rows || []).forEach((row) => {
        ['H', 'A', 'L'].forEach((type) => {
          ...
          const stats = rowStats(row, type);
          if (stats.unlocked === false) return;
          const key = `${titleKey(row.title)}|${type}`;
          if (isSuppressedTrackerOnlyChart(tableName, row.title, type)) return;
          if (!isAdmin && pendingKeys.has(key)) return;
...
export function collectChartDiscoveryCandidates(rankTables, rows) {
  const known = new Set();
  Object.entries(rankTables || {}).forEach(([tableKey, tableData]) => {
    (tableData?.categories || []).forEach((category) => {
      (category?.items || []).forEach((item) => {
        const title = String(item?.data?.title || '').trim();
        const type = String(item?.data?.type || 'A').trim().toUpperCase();
        if (!title || !/^[HAL]$/.test(type)) return;
        known.add(`${String(tableKey || '').trim()}|${titleKey(title)}|${type}`);
      });
    });
  });
  ...
  (rows || []).forEach((row) => {
    ...
    ['H', 'A', 'L'].forEach((type) => {
      ...
      if (noteCount <= 0) return;
      const rating = num(row?.[`${prefix} Rating`]);
      if (![10, 11, 12].includes(rating)) return;
      const tableKey = `SP${rating}H`;
      if (isSuppressedTrackerOnlyChart(tableKey, title, type)) return;
      const key = `${tableKey}|${titleKey(title)}|${type}`;
      ...
      candidates.push({
        ...
        level: rating,
        songTitle: title,
        normalizedTitle: titleKey(title),
        chartType: type,
        noteCount,
        rating
      });
...
export function sortItems(items, sortMode) {
  ...
  out.sort((a, b) => {
    ...
    if (sortMode === 'clear' || sortMode === 'lamp') {
      const lampDiff = (CLEAR_SORT_ORDER[b.clearStatus] ?? 0) - (CLEAR_SORT_ORDER[a.clearStatus] ?? 0);
      if (lampDiff !== 0) return lampDiff;
    } else if (sortMode === 'score') {
      const scoreDiff = (SCORE_GRAPH_ORDER.indexOf(b.scoreTier || 'NOPLAY')) - (SCORE_GRAPH_ORDER.indexOf(a.scoreTier || 'NOPLAY'));
      if (scoreDiff !== 0) return scoreDiff;
      const rateDiff = Number(b.rate || 0) - Number(a.rate || 0);
      if (rateDiff !== 0) return rateDiff;
      const exDiff = Number(b.exScore || 0) - Number(a.exScore || 0);
