export function createInteractionController({
  ...
  function goalPayloadFromForm(prefix = 'goal', chart = null) {
    ...
    else if (kind === 'RATE') payload.targetRate = Math.max(0, Math.min(100, Math.round(Number($(`${prefix}Rate`)?.value || 0) * 100) / 100));
  ...
  async function openPeerRankViewer() {
    const menu = $('socialPeerMenu');
    const peer = {
      peer_user_id: menu?.dataset.peerUserId || '',
      dj_name: menu?.dataset.peerDjName || '',
      infinitas_id: menu?.dataset.peerInfinitasId || '',
      isMutual: menu?.dataset.peerMutual === 'true'
    };
    menu?.classList.add('hidden');
    if (!peer.peer_user_id || !peer.isMutual) return showToast('맞팔인 상대의 서열표만 볼 수 있습니다.');
    try {
      await withBusyOverlay(
        '서열표 불러오는 중...',
        '맞팔 상대의 서열표 데이터를 준비하고 있습니다.',
        async () => {
          let result;
          try {
            result = await rpc('get_mutual_rank_tracker_rows', { p_peer_user_id: peer.peer_user_id });
          } catch (error) {
            if (!String(error?.message || error || '').includes('get_mutual_rank_tracker_rows')) throw error;
            result = await rpc('get_follow_tracker_rows', { p_peer_user_id: peer.peer_user_id });
          }
          const row = Array.isArray(result) ? result[0] : result;
          if (!row) throw new Error('rank_table_share_disabled_or_not_mutual');
          const trackerRows = Array.isArray(row?.tracker_rows) ? row.tracker_rows : [];
          state.peerRankViewer = {
            ...state.peerRankViewer,
            open: true,
            peerUserId: peer.peer_user_id,
            peerLabel: row?.dj_name || peer.dj_name || '상대',
            peerInfinitasId: row?.infinitas_id || peer.infinitas_id || '',
            rows: trackerRows,
            tableViews: buildViews(state.rankTables, state.songRadarCatalog, trackerRows),
            activeTable: state.peerRankViewer?.activeTable || 'SP11H',
            viewMode: state.peerRankViewer?.viewMode || 'normal',
            sortMode: state.peerRankViewer?.sortMode || 'lamp',
            searchQuery: ''
          };
        }
      );
      ...
      showToast(`상대 서열표 불러오기 실패: ${error.message || error}`);
    }
  }

  function closePeerRankViewer(options = {}) {
    state.peerRankViewer = {
      ...state.peerRankViewer,
      open: false
    };
  ...
  function setPeerRankTable(tableKey) {
    if (!state.peerRankViewer?.open || !state.peerRankViewer.tableViews?.[tableKey]) return;
    state.peerRankViewer.activeTable = tableKey;
    render();
  }

  function setPeerRankView(viewMode) {
    if (!state.peerRankViewer?.open) return;
    state.peerRankViewer.viewMode = viewMode === 'wide' ? 'wide' : 'normal';
    render();
  }

  function setPeerRankSort(sortMode) {
    if (!state.peerRankViewer?.open) return;
    state.peerRankViewer.sortMode = sortMode === 'score' ? 'score' : 'lamp';
    render();
  }

  function setPeerRankSearch(searchQuery) {
    if (!state.peerRankViewer?.open) return;
    state.peerRankViewer.searchQuery = String(searchQuery || '');
    render();
  }
  ...
  async function exportImage() {
    ...
    const captureOptions = {
      ...
      scale: 2,
      onclone: (doc) => {
        const clonedExportArea = doc.getElementById('exportArea');
        if (!clonedExportArea) return;
        clonedExportArea.style.removeProperty('--mobile-rank-base-width');
        clonedExportArea.style.removeProperty('--mobile-rank-scale');
        clonedExportArea.style.width = '';
        clonedExportArea.style.minWidth = '';
        clonedExportArea.style.maxWidth = '';
        clonedExportArea.style.transform = 'none';
        clonedExportArea.style.marginBottom = '';
      }
    };
    if (window.matchMedia('(max-width: 600px)').matches) {
      captureOptions.windowWidth = 1200;
      captureOptions.windowHeight = Math.max(900, exportArea?.scrollHeight || 0);
    }
    const canvas = await window.html2canvas(exportArea, captureOptions);
