  ...
  async function saveBingoDraft() {
    ...
    await withBusyOverlay(
      ...
      async () => {
        ...
        syncGoalStoreFromBingoDraft();
        persistBingoDraftCache();
        await saveProfileToCloud(state.auth.user, state.profile, 'bingo-draft-save');
        render();
        showToast('빙고 임시 저장이 완료되었습니다.');
      }
    );
  }
  ...
  async function cancelBingoDraft() {
    ...
    await withBusyOverlay(
      ...
      async () => {
        ...
        syncGoalStoreFromBingoDraft();
        clearBingoDraftCache();
        await saveProfileToCloud(state.auth.user, state.profile, 'bingo-delete');
        render();
  ...
  async function publishBingo() {
    ...
    await withBusyOverlay(
      ...
      async () => {
        ...
        syncGoalStoreFromBingoDraft();
        persistBingoDraftCache();
        await saveProfileToCloud(state.auth.user, state.profile, 'bingo-publish');
        render();
        showToast('빙고 저장이 완료되었습니다.');
      }
    );
  }
