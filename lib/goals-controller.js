export function createGoalsController(env) {
  ...
  async function persistBingoProfile(reason, options = {}) {
    const timeoutMs = Math.max(1000, Number(options.timeoutMs || 8000));
    const savePromise = saveProfileToCloud(state.auth.user, state.profile, reason);
    if (options.allowSoftTimeout !== true) {
      await savePromise;
      return 'saved';
    }
  ...
  async function saveBingoDraft() {
    ...
    await withBusyOverlay(
      ...
      async () => {
        ...
        syncGoalStoreFromBingoDraft();
        persistBingoDraftCache();
        const saveStatus = await persistBingoProfile('bingo-draft-save', { allowSoftTimeout: true });
        render();
        showToast(saveStatus === 'delayed'
          ? '빙고 임시 저장 반영이 지연되고 있습니다. 화면은 먼저 갱신했습니다.'
          : '빙고 임시 저장이 완료되었습니다.');
      }
    );
  }
  ...
  async function cancelBingoDraft() {
    ...
    await withBusyOverlay(
      ...
      async () => {
        ...
        syncGoalStoreFromBingoDraft();
        clearBingoDraftCache();
        const saveStatus = await persistBingoProfile('bingo-delete', { allowSoftTimeout: true });
        render();
        showToast(saveStatus === 'delayed'
          ? '빈 빙고 취소는 반영했고, 서버 저장은 지연되고 있습니다.'
          : '작성 중인 빈 빙고를 취소했습니다.');
      }
    );
  }
  ...
  async function publishBingo() {
    ...
    await withBusyOverlay(
      ...
      async () => {
        ...
        syncGoalStoreFromBingoDraft();
        persistBingoDraftCache();
        const saveStatus = await persistBingoProfile('bingo-publish', { allowSoftTimeout: true });
        render();
        showToast(saveStatus === 'delayed'
          ? '빙고 저장 반영이 지연되고 있습니다. 화면은 먼저 갱신했습니다.'
          : '빙고 저장이 완료되었습니다.');
      }
    );
  }
