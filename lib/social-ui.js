...
function feedTypeClass(type) {
  ...
  if (type === 'chart_discovery_candidate') return 'feed-history';
  return '';
}
...
function feedEventToItem(row) {
  ...
  if (type === 'chart_discovery_candidate') {
    const title = String(payload.song_title || '-');
    const table = String(payload.table_key || '');
    const chartType = String(payload.chart_type || '');
    const candidateId = String(payload.candidate_id || '');
    return {
      ...common,
      title: '신규 곡 후보',
      body: `${actorLabel} 갱신에서 ${table} ${title} [${chartType}] 후보가 발견되었습니다.`,
      candidateId
    };
  }
  ...
  feed.innerHTML = feedItems.length
    ? feedItems.slice(0, 60).map((item, index) => {
      ...
      const bingoActionHtml = item.type === 'bingo_transfer_received'
        ...
        : '';
      const discoveryActionHtml = item.type === 'chart_discovery_candidate' && item.candidateId
        ? `<button type="button" class="social-feed-action-btn accept" data-discovery-promote="${esc(item.candidateId)}">pending 등록</button>`
        : '';
      return `<article class="social-feed-post ${feedTypeClass(item.type)}" style="--feed-order:${index}" data-feed-id="${esc(item.id)}">
        ...
        <div class="social-feed-actions">${followActionHtml}${bingoActionHtml}${discoveryActionHtml}</div>
      </article>`;
    }).join('')
