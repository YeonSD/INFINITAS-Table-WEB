...
function followUserRowHtml(peer, { isMutual = false, allowFollowBack = false } = {}) {
  ...
  return `<div class="social-follow-user social-follow-user-following${isMutual ? ' is-mutual' : ''}">
    ${mutualBadge}
    <button type="button" class="social-avatar social-avatar-plain" data-peer-avatar="${esc(peerId)}" data-peer-dj-name="${esc(djName)}" data-peer-infinitas-id="${esc(infinitasId)}">${avatarHtml}</button>
    <div>
...
export function songSocialSectionHtml(rows) {
  ...
  const orderedList = [...rows].sort((a, b) => {
    ...
    return `<div class="song-follow-line"><strong>${esc(row.dj_name || '?대쫫 ?놁쓬')}</strong> | 램프: ${esc(lamp)} | 랭크: ${esc(rank)} | 점수: ${score} | 달성률: ${rate.toFixed(2)}%</div>`;
  }).join('');
  return `<hr /><div><strong>?붾줈???꾪솴</strong>${orderedList}</div>`;
