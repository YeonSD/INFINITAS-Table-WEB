...
import { renderPeerRankViewer } from './peer-rank-ui.js';
...
import { getDeferredPanelRenderers } from './render-plan.js';
...
function syncSlidingControls() {
  ...
  requestAnimationFrame(() => slidingGroups.forEach((fn) => fn()));
}
...
export function renderApp(ctx) {
  ...
  safe('peerRankViewer', () => renderPeerRankViewer(ctx));
  renderDeferredPanel(ctx, ctx.state?.activePanel || 'rank');
  safe('controls', () => syncActiveControls(ctx.state));
  decorateButtons();
}

export function renderDeferredPanel(ctx, panel = 'rank') {
  const safe = (label, fn) => {
    try {
      fn();
    } catch (error) {
  ...
  const renderers = {
    ...
    goalCandidates: () => renderGoalCandidates(ctx, enhancedSelects),
    goals: () => renderGoals(ctx, enhancedSelects),
    songGoalBingoPicker: () => renderSongGoalBingoPicker(ctx),
    social: () => renderSocialPanel(ctx, renderSocialHistoryPopup)
  };
  getDeferredPanelRenderers(panel).forEach((key) => {
    const renderPanel = renderers[key];
    if (renderPanel) safe(key, renderPanel);
  });
}
...
export function showSongPopup(chart, event, isAuthorized, options = {}) {
  ...
  const editBtn = options.isAdmin ? `<button type="button" class="song-meta-open-btn" data-song-meta-edit="${esc(chart.key)}">정보 수정</button>` : '';
  const pendingBtn = options.isAdmin && chart.isPendingRelease
    ? `<button type="button" class="song-pending-apply-btn" data-song-pending-apply="${esc(chart.key)}">서열표에 적용</button>`
    : '';
  const pendingBadge = options.isAdmin && chart.isPendingRelease
    ? '<span class="song-popup-pending-badge">미출시 후보</span>'
    : '';
  $('popupTitle').innerHTML = `<span class="song-popup-headline">${esc(`${chart.title} [${chart.type}]`)}${pendingBadge}</span><span class="song-popup-actions">${goalBtn}${pendingBtn}${editBtn}</span>`;
  ...
  const pendingHtml = options.isAdmin && chart.isPendingRelease
    ? '<div class="song-popup-pending-note">일반 유저에게는 숨김 처리된 미출시 후보 곡입니다. 실제 업데이트가 확인되면 서열표에 적용한 뒤 변경사항을 배포하세요.</div><hr />'
    : '';
  $('popupMeta').innerHTML = `${pendingHtml}${radarHtml}<hr />${metaHtml}<hr />${playHtml}${songSocialSectionHtml(options.songSocialRows || [])}`;
  popup.classList.remove('hidden');
  positionPopup(popup, event);
}
...
function openSocialPeerMenuFromAvatar(btn, event) {
  const menu = $('socialPeerMenu');
  if (!menu || !btn) return;
  const rect = btn.getBoundingClientRect();
  const isMutual = btn.getAttribute('data-peer-mutual') === 'true';
  ...
  menu.dataset.peerMutual = isMutual ? 'true' : 'false';
  ...
  $('btnSocialPeerRank')?.classList.toggle('hidden', !isMutual);
...
export function bindUi(ctx) {
  ...
  $('goalKind')?.addEventListener('change', () => syncGoalTargetInputVisibility('goal', enhancedSelects));
  ...
  $('socialMobileTabsHost')?.addEventListener('click', (e) => {
    const btn = e.target.closest('[data-social-mobile-section]');
    if (btn) ctx.actions.setSocialMobileSection(btn.getAttribute('data-social-mobile-section') || 'feed');
  });
  $('socialFeed')?.addEventListener('click', (e) => {
    ...
    const discoveryPromote = e.target.closest('[data-discovery-promote]');
    if (discoveryPromote) return ctx.actions.promoteDiscoveryCandidate(discoveryPromote.getAttribute('data-discovery-promote') || '');
  });
  $('socialMyCard')?.addEventListener('click', (e) => {
    ...
    const followersBtn = e.target.closest('[data-open-followers]');
    if (followersBtn) {
      const rect = followersBtn.getBoundingClientRect();
      if ($('socialFollowersPopup')?.classList.contains('hidden')) ctx.actions.openFollowersPopup(rect);
      else ctx.actions.closeFollowersPopup();
    }
  });
  ...
  $('socialFollowList')?.addEventListener('click', (e) => {
    ...
    if (!btn) return;
    openSocialPeerMenuFromAvatar(btn, e);
  });
  $('socialFollowersPopup')?.addEventListener('click', (e) => {
    const closeBtn = e.target.closest('[data-close-followers-popup]');
    if (closeBtn) return ctx.actions.closeFollowersPopup();
    const followBtn = e.target.closest('[data-follow-target]');
    if (followBtn) {
      const peerUserId = followBtn.getAttribute('data-follow-target');
      const peerLabel = followBtn.getAttribute('data-follow-label') || '상대';
      if (window.confirm(`${peerLabel}님에게 팔로우 요청을 보내시겠습니까?`)) {
        ctx.actions.sendFollow(peerUserId, { peerLabel, closeSearchDialog: false });
      }
      return;
    }
    const btn = e.target.closest('[data-peer-avatar]');
  ...
  $('btnSocialPeerRank')?.addEventListener('click', ctx.actions.openPeerRankViewer);
  $('btnSocialPeerUnfollow')?.addEventListener('click', ctx.actions.unfollowPeer);
  $('peerRankDialog')?.addEventListener('click', (e) => {
    const closeBtn = e.target.closest('#peerRankCloseTop');
    if (closeBtn) return ctx.actions.closePeerRankViewer();
    const tableBtn = e.target.closest('[data-peer-rank-table]');
    if (tableBtn) return ctx.actions.setPeerRankTable(tableBtn.getAttribute('data-peer-rank-table') || '');
    const viewBtn = e.target.closest('[data-peer-rank-view]');
    if (viewBtn) return ctx.actions.setPeerRankView(viewBtn.getAttribute('data-peer-rank-view') || 'normal');
    const sortBtn = e.target.closest('[data-peer-rank-sort]');
    if (sortBtn) return ctx.actions.setPeerRankSort(sortBtn.getAttribute('data-peer-rank-sort') || 'lamp');
  });
  $('peerRankDialog')?.addEventListener('close', () => ctx.actions.closePeerRankViewer({ skipDialogClose: true }));
  $('peerRankDialog')?.addEventListener('input', (e) => {
    if (e.target?.id === 'peerRankSearch') ctx.actions.setPeerRankSearch(e.target.value || '');
  });
  ...
  $('btnPublishSnapshot')?.addEventListener('click', ctx.actions.publishSnapshotChanges);
  $('btnRefreshDiscoveryCandidates')?.addEventListener('click', () => ctx.actions.refreshChartDiscoveryCandidates({ silent: false }));
  $('settingsAdminDiscoveryList')?.addEventListener('click', (e) => {
    const btn = e.target.closest('[data-discovery-promote]');
    if (btn) ctx.actions.promoteDiscoveryCandidate(btn.getAttribute('data-discovery-promote') || '');
  });
  ...
  $('songPopup')?.addEventListener('click', (e) => {
    const btn = e.target.closest('[data-song-meta-edit]');
    if (btn) ctx.actions.openSongMetaEditor(btn.getAttribute('data-song-meta-edit') || '');
  });
  $('songPopup')?.addEventListener('click', (e) => {
    const btn = e.target.closest('[data-song-pending-apply]');
    if (btn) ctx.actions.applyPendingChartRelease(btn.getAttribute('data-song-pending-apply') || '');
  });
  ...
  $('songMetaCategory')?.addEventListener('change', () => ctx.actions.syncSongMetaSortIndexFromCategory());
  $('songMetaSortIndex')?.addEventListener('input', (e) => {
    if (e.target) e.target.dataset.touched = 'true';
  });
  $('songMetaEditorCancelBtn')?.addEventListener('click', () => ctx.actions.closeSongMetaEditor());
  $('songMetaEditorCloseTop')?.addEventListener('click', () => ctx.actions.closeSongMetaEditor());
  $('songMetaEditorDeleteBtn')?.addEventListener('click', () => ctx.actions.deleteSongMetaEditor());
  $('songMetaEditorForm')?.addEventListener('submit', (e) => {
    e.preventDefault();
    ctx.actions.saveSongMetaEditor();
  });
  $('songMetaEditorDialog')?.addEventListener('close', () => ctx.actions.closeSongMetaEditor({ skipDialogClose: true }));
  ...
  document.addEventListener('click', (e) => {
    ...
    if (!e.target.closest('#socialFollowersPopup') && !e.target.closest('[data-open-followers]')) ctx.actions.closeFollowersPopup();
