...
export function bindUi(ctx) {
  ...
  const refreshSocialIfVisible = () => {
    if (document.visibilityState === 'hidden') return;
    if (ctx.state.activePanel !== 'social' || !ctx.isAuthorized()) return;
    ctx.actions.refreshSocial?.();
  };
  document.addEventListener('visibilitychange', refreshSocialIfVisible);
  window.addEventListener('focus', refreshSocialIfVisible);
  ...
  $('socialFeed')?.addEventListener('click', (e) => {
    ...
    if (accept) return ctx.actions.respondFollow(accept.getAttribute('data-feed-accept'), true, {
      eventId: accept.getAttribute('data-feed-event-id'),
      peerUserId: accept.getAttribute('data-feed-peer-user-id'),
      peerLabel: accept.getAttribute('data-feed-peer-label')
    });
    ...
    if (reject) return ctx.actions.respondFollow(reject.getAttribute('data-feed-reject'), false, {
      eventId: reject.getAttribute('data-feed-event-id'),
      peerUserId: reject.getAttribute('data-feed-peer-user-id'),
      peerLabel: reject.getAttribute('data-feed-peer-label')
    });
  ...
  $('socialSearchResult')?.addEventListener('click', (e) => {
    ...
    if (btn) ctx.actions.sendFollow(
      btn.getAttribute('data-follow-target'),
      btn.getAttribute('data-follow-label')
    );
  });
