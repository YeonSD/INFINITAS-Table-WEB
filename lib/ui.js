...
export function renderApp(ctx) {
  ...
  const activePanel = String(ctx?.state?.activePanel || 'rank');
  safe('account', () => renderAccountInfo(ctx));
  safe('medals', () => renderMedals(ctx));
  if (activePanel === 'rank') safe('rank', () => renderRankTable(ctx));
  if (activePanel === 'graphs') safe('graphs', () => renderGraphs(ctx, legendTextColor, CLEAR_COLORS, SCORE_GRAPH_ORDER, SCORE_COLORS));
  if (activePanel === 'history') safe('history', () => renderHistory(ctx));
  ...
  if (activePanel === 'goals' || ctx?.state?.songGoalPickerPayload) {
  ...
  if (activePanel === 'social' || ctx?.state?.socialHistoryPopup?.open) {
