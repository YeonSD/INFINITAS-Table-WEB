...
function rankTableDisplayMode(ctx) {
  return ctx.activeSocialSettings?.().rankTableDisplay === 'rate' ? 'rate' : 'rank';
}

function songBadgeText(chart, displayMode) {
  if (displayMode === 'rate') {
    if (chart.clearStatus === 'NOPLAY') return '';
    return `${Number(chart.rate || 0).toFixed(2)}%`;
  }
  return chart.scoreTier || '';
}
...
export function renderRankTable(ctx) {
  ...
  const displayMode = rankTableDisplayMode(ctx);
  ...
  container.innerHTML = orderedCategories.map((cat) => {
    ...
    return `<table class="category-table"><tbody>${rows.map((row, rowIndex) => `<tr>${rowIndex === 0 ? `<th class="category-label" style="--folder-lamp-color:${color}" rowspan="${rows.length}" title="폴더 최저 램프: ${esc(tier)}">${esc(cat.name)}</th>` : ''}${row.map((chart) => {
      const badgeText = songBadgeText(chart, displayMode);
      return `<td class="song-cell"><button class="${btnClass(chart)}" data-chart-key="${esc(chart.key)}" title="${esc(chart.title)}"><span class="song-title">${esc(trunc(chart.title, cols))}</span>${badgeText ? `<span class="score-badge">${esc(badgeText)}</span>` : ''}</button></td>`;
    }).join('')}${Array.from({ length: Math.max(0, cols - row.length) }).map(() => '<td class="empty-cell"></td>').join('')}</tr>`).join('')}</tbody></table>`;
  }).join('');
}
