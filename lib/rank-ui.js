...
function isCompactRankViewport() {
  return typeof window !== 'undefined' && window.matchMedia?.('(max-width: 600px)').matches;
}

function mobileRankCardHtml(chart) {
  const rank = chart.scoreTier ? `<span class="mobile-rank-badge">${esc(chart.scoreTier)}</span>` : '';
  const lamp = chart.clearStatus || chart.lamp || 'NOPLAY';
  const score = Number(chart.exScore || 0);
  const rate = Number(chart.rate || 0);
  return `<button class="${btnClass(chart)} mobile-rank-card" data-chart-key="${esc(chart.key)}" title="${esc(chart.title)}">
    <span class="mobile-rank-card-main">
      <span class="mobile-rank-title">${esc(chart.title)}</span>
      <span class="mobile-rank-type diff-${esc(chart.type)}">${esc(chart.type || '-')}</span>
    </span>
    <span class="mobile-rank-card-meta">
      <span>${esc(lamp)}</span>
      ${rank}
      <span>EX ${score}</span>
...
export function renderRankTable(ctx) {
  ...
  if (isCompactRankViewport()) {
    container.innerHTML = orderedCategories.map((cat) => {
      ...
      return `<section class="mobile-rank-category" style="--folder-lamp-color:${color}">
        <div class="mobile-rank-category-head" title="Folder lowest lamp: ${esc(tier)}">
          <span>${esc(cat.name)}</span>
          <span>${items.length}</span>
        </div>
        <div class="mobile-rank-card-list">${items.map(mobileRankCardHtml).join('')}</div>
      </section>`;
    }).join('');
    return;
  }
