...
function normalizedBingoDraft(profile) {
  const rawBingo = profile?.bingoState || {};
  const rawDraft = rawBingo?.draft || {};
  const size = [3, 4, 5].includes(Number(rawDraft.size || 3)) ? Number(rawDraft.size || 3) : 3;
  const cells = Array.isArray(rawDraft.cells) ? rawDraft.cells.slice(0, size * size) : [];
  while (cells.length < size * size) cells.push(null);
  const savedBoards = (Array.isArray(rawBingo.savedBoards) ? rawBingo.savedBoards : [])
    .filter((board) => board && typeof board === 'object')
    .slice(0, 5);
  const activeBoardId = String(rawBingo.activeBoardId || '').trim();
  const published = activeBoardId
    ? savedBoards.find((board) => String(board?.id || '') === activeBoardId) || null
    : null;
  return {
    bingo: {
      ...rawBingo,
      savedBoards,
      activeBoardId: published?.id || activeBoardId,
      published,
      draft: {
        size,
        cells,
        updatedAt: String(rawDraft.updatedAt || '')
      }
    },
    draft: {
      size,
...
function renderGoalsFallback(ctx, error) {
  const list = $('goalList');
  if (!list) return;
  const { bingo, draft } = normalizedBingoDraft(ctx.state.profile);
...
export function renderGoals(ctx, enhancedSelects = new Map()) {
  ...
  try {
    ...
    const { bingo, draft, savedBoards, published } = normalizedBingoDraft(ctx.state.profile);
...
export function renderSongGoalBingoPicker(ctx) {
  ...
  const rawBingo = ctx.state.profile?.bingoState || {};
  const rawDraft = rawBingo?.draft || {};
  const size = [3, 4, 5].includes(Number(rawDraft.size || 3)) ? Number(rawDraft.size || 3) : 3;
  const cells = Array.isArray(rawDraft.cells) ? rawDraft.cells.slice(0, size * size) : [];
  while (cells.length < size * size) cells.push(null);
