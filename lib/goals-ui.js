...
import { normalizeBingoStateShape } from './bingo-state.js';
...
function normalizedBingoDraft(profile) {
  const bingo = normalizeBingoStateShape(profile?.bingoState || {});
  if (profile && typeof profile === 'object') profile.bingoState = bingo;
  return {
    bingo,
    draft: bingo.draft,
    savedBoards: bingo.savedBoards,
    published: bingo.published
  };
}
