...
function renderGoalsFallback(ctx, error) {
  ...
  const cellsHtml = displayBoard.cells.map((goal, index) => {
    ...
    const target = goal.kind === 'RANK'
      ...
      : goal.kind === 'RATE'
        ? `RATE : ${esc(bingoGoalTargetText(goal))}`
      : goal.kind === 'SCORE'
...
export function syncGoalTargetInputVisibility(prefix = 'goal', enhancedSelects = new Map()) {
  ...
  $(`${prefix}Rate`)?.classList.toggle('hidden', hideRate);
  if (prefix === 'goal') {
...
export function renderGoals(ctx, enhancedSelects = new Map()) {
  ...
  try {
    ...
    const hasVisibleBoard = !!published || !!draft.updatedAt;
    $('goalPool').innerHTML = '';
    $('bingoMeta').innerHTML = '';
    ...
    if ($('btnBingoDraftSave')) $('btnBingoDraftSave').disabled = false;
    ...
    const draftListCard = bingo.draft.updatedAt
      ? `<button type="button" class="bingo-list-item draft ${!published ? 'active' : ''}" data-bingo-board-select="" style="--i:-1">
          <div class="bingo-list-name">[빙고 작성]</div>
          <div class="bingo-list-meta">${esc(`${draft.size} x ${draft.size}`)} / 수정 중</div>
        </button>`
      : '';
    const emptyMainHtml = !hasVisibleBoard
      ? `<div class="bingo-empty-state">
           <div class="bingo-empty-title">보유한 빙고가 없습니다.</div>
           <div class="bingo-empty-copy">새 빙고를 생성하거나 저장된 빙고를 불러오세요.</div>
         </div>`
      : '';
    const boardActionsHtml = boardMode === 'draft'
      ? `<button type="button" class="small-btn" data-bingo-header-action="draft-save">임시 저장</button>
    ...
    list.innerHTML = `<div class="bingo-layout">
      <aside class="bingo-sidebar">
        ...
        <div class="bingo-sidebar-panel">
          ...
          <button type="button" class="small-btn bingo-sidebar-import-btn" data-bingo-header-action="import">빙고 가져오기</button>
        </div>
      </aside>
      <div class="bingo-layout-main">
        <div class="bingo-board-shell">
          <div class="bingo-board-head">
            <div class="bingo-board-headline">${published?.name ? esc(published.name) : (hasVisibleBoard ? '칸을 선택한 후 목표를 설정하세요.' : '빙고 없음')}</div>
            <div class="bingo-board-head-actions">
              ${hasVisibleBoard ? boardActionsHtml : ''}
              ${isBoardComplete ? '<div class="bingo-finished-banner">BINGO!</div>' : ''}
            </div>
          </div>
          ${emptyMainHtml || `<div class="bingo-board-grid size-${displayBoard.size || 3}">
            ${cellsHtml}
          </div>`}
        </div>
      </div>
